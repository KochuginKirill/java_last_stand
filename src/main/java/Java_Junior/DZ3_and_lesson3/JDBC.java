package Java_Junior.DZ3_and_lesson3;
...
import static Java_Junior.DZ3_and_lesson3.hw.Homework.getPersonDepartmentName;
...
public class JDBC {
  ...
  public static void main(String[] args) {
    ...
    try (Connection connection = DriverManager.getConnection("jdbc:h2:mem:test")) {
      ...
      System.out.println("------------------------------------------");
      selectDepartmentData(connection);
      System.out.println("------------------------------------------");
  ...
  private static void createTable(Connection connection) throws SQLException {
    try (Statement statement = connection.createStatement()) {
      statement.execute("""
        ...
        """);
      //foreign key bigint department_id REFERENCES department (id)
    } catch (SQLException e) {
  ...
  private static void insertDepartmentData(Connection connection) throws SQLException {
    try (Statement statement = connection.createStatement()) {
      StringBuilder insertQuery = new StringBuilder("insert into department(id, departmentName) values\n");
      for (int i = 1; i <= 3; i++) {
  ...
  private static void insertData(Connection connection) throws SQLException {
    try (Statement statement = connection.createStatement()) {
      StringBuilder insertQuery = new StringBuilder("insert into person(id, name, age, active, departmentId) values\n");
      for (int i = 1; i <= 10; i++) {
        int age = ThreadLocalRandom.current().nextInt(20, 60);
        boolean active = ThreadLocalRandom.current().nextBoolean();
        Random rand = new Random();
        insertQuery.append(String.format("(%s, '%s', %s, %s, %s)", i, "Person #" + i, age, active, rand.nextInt(1, 3)));
...
//    }
  ...
  private static void selectData(Connection connection) throws SQLException {
    try (Statement statement = connection.createStatement()) {
      ResultSet resultSet = statement.executeQuery("""
        select id, name, age, departmentId
        from person
      ...
      while (resultSet.next()) {
        ...
        System.out.println("Найдена строка: [id = " + id + ", name = " + name + ", age = " + age + ", departmentId = " + departmentId + "]");
      }
    }
  }
  private static void selectDepartmentData(Connection connection) throws SQLException {
    try (Statement statement = connection.createStatement()) {
      ResultSet resultSet = statement.executeQuery("""
